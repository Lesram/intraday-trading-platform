...
class SlippageModel:
    ...
    def estimate_slippage(
        self, symbol: str, quantity: float, price: float, daily_volume: float = 1000000
    ) -> float:
...
class CommissionModel:
    ...
    def calculate_commission(
        self, symbol: str, quantity: float, price: float, side: str
    ) -> float:
...
class FillSimulatorAdapter:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(
                "⚠️ ExecutionSimulator not available, using built-in simulation"
            )

        logger.info("🎮 FillSimulatorAdapter initialized")

    async def simulate_fill(
        self, symbol: str, side: str, qty: float, price: float, **kwargs
    ) -> SimulatedFill:
        """
        ...
        try:
                ...
                fill = SimulatedFill(
                    ...
                    metadata={
                        ...
                        "signal_confidence": getattr(
                            trade_result, "signal_confidence", 0.0
                        ),
            ...
            logger.info(
                f"🎮 Simulated fill: {symbol} {side} {qty} @ {fill.executed_price:.4f}"
            )
            return fill
    ...
    async def _simulate_fill_builtin(
        self,
        order_id: str,
        symbol: str,
        side: str,
    ...
    async def simulate_fill_with_delay(
        self,
        symbol: str,
        side: str,
        qty: float,
    ...
    def get_fill_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "slippage_model": {
                "base_slippage_bps": {
                    k: v * 10000 for k, v in self.slippage_model.base_slippage.items()
                },
    ...
    async def get_market_impact_estimate(
        self, symbol: str, qty: float, price: float, daily_volume: float = 1000000
    ) -> dict[str, float]:
        """Get market impact estimates for order sizing"""
        slippage = self.slippage_model.estimate_slippage(
            symbol, qty, price, daily_volume
        )
        commission = self.commission_model.calculate_commission(
...
async def test_fill_simulator_adapter():
    ...
    fill = await adapter.simulate_fill(
        symbol="AAPL", side="buy", qty=100.0, price=150.0, volume=5000000
    )

    print(
        f"✅ Basic fill: {fill.symbol} {fill.side} {fill.quantity} @ {fill.executed_price:.4f}"
    )
    ...
    print(
        f"✅ Market impact: {impact['slippage_bps']:.1f}bps, Cost: {impact['total_cost_pct']:.3f}%"
    )
    ...
    print(
        f"✅ Statistics: {stats['adapter_type']}, ExecutionSim: {stats['has_execution_simulator']}"
    )
