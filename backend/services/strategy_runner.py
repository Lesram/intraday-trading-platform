...
class StrategyRunner:
    ...
    async def generate_signals(self, symbol: str, market_data: Dict[str, Any]) -> List[SignalEvent]:
        """
    ...
    async def run_once(self, symbol: str, market_data: Dict[str, Any]) -> List[OrderEvent]:
        """
            ...
            for signal in signals:
                try:
                    ...
                    if risk_decision.decision.value == "BLOCKED":
                        primary_reason = risk_decision.reasons[0] if risk_decision.reasons else "unknown"
                        severity = "high" if "limit" in primary_reason.lower() else "medium"
                        metrics.record_risk_block(primary_reason, signal.symbol, severity)
                    elif risk_decision.decision.value == "MODIFIED":
                        metrics.record_risk_modification(signal.symbol, "quantity_reduced")
                    ...
                    if risk_decision.decision.value in ["APPROVED", "MODIFIED"]:
                        order_event = await self._execute_order(signal, risk_decision.approved_qty)
                        if order_event:
    ...
    async def _execute_order(self, signal: SignalEvent, approved_qty: float) -> Optional[OrderEvent]:
        ...
        try:
            ...
            logger.info(f"📈 Order executed: {signal.symbol} {signal.side} {approved_qty} @ {signal.price}")
            return order_event

        except Exception as e:
            logger.error(f"❌ Order execution failed: {e}")
            metrics.record_order_rejected(signal.symbol, str(e)[:50])  # Truncate error message
            return None

    async def run_loop(self, symbols: List[str], data_source_callback, interval_seconds: int = 60):
        """
        ...
        """
        self._running = True
        logger.info(f"🔄 Starting strategy loop for {symbols} (interval: {interval_seconds}s)")
    ...
    def start_loop(self, symbols: List[str], data_source_callback, interval_seconds: int = 60):
