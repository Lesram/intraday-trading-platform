...
class RiskService:
    ...
    def check_position_risk(
        ...
        if quantity > self.max_position_size:
            risks.append(f"Position size {quantity} exceeds maximum {self.max_position_size}")
            
        if portfolio_heat > self.max_portfolio_heat:
            risks.append(f"Portfolio heat {portfolio_heat:.2%} exceeds maximum {self.max_portfolio_heat:.2%}")
        ...
        result = {
            ...
            "max_quantity": min(self.max_position_size, int(current_portfolio_value * self.max_portfolio_heat / price))
        }
        
        if approved:
            logger.info(f"✅ Risk check passed for {symbol}: {quantity} shares @ ${price}")
        else:
    ...
    def calculate_position_size(
        ...
        logger.info(f"📊 Position size for {symbol}: {position_size} shares (risk: ${risk_amount:.2f})")
