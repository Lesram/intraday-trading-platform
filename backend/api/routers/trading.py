...
class OrderRequest(BaseModel):
    ...
    order_type: str = Field(
        default="market", description="Order type: 'market' or 'limit'"
    )
...
async def submit_order(
    order: OrderRequest, background_tasks: BackgroundTasks, auth_token: RequireAuth
) -> OrderResponse:
    """
    ...
    """
    logger.info(
        f"📊 Authenticated order submission: {order.symbol} {order.side} {order.quantity}"
    )
...
async def cancel_order(order_id: str, auth_token: RequireAuth) -> dict[str, Any]:
    """
...
async def get_orders(
    symbol: str | None = None, status: str | None = None, auth_token: RequireAuth = None
) -> dict[str, Any]:
    """
    ...
    return {
        "orders": orders,
        "count": len(orders),
        "filtered_by": {"symbol": symbol, "status": status},
    }


async def process_order_async(
    order_id: str, order_data: dict[str, Any], auth_token: str
):
    """Background task for order processing"""
    try:
