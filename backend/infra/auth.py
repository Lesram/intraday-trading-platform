...
async def verify_bearer_token(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security_bearer)) -> str:
...
async def verify_trading_auth(
    ...
    if not api_key and not bearer_token:
        ...
        raise AuthError("Authentication required. Provide X-API-Key header or Authorization: Bearer token")
...
async def log_auth_attempt(request, call_next):
    ...
    if is_protected:
        ...
        logger.info(f"🔐 Auth attempt on {request.url.path} - API key: {'✅' if api_key else '❌'}, Bearer: {'✅' if auth_header.startswith('Bearer ') else '❌'}")
    
    response = await call_next(request)
    
    process_time = time.time() - start_time
    if is_protected and hasattr(response, 'status_code') and response.status_code == 401:
