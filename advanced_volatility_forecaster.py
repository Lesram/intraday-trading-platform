...
class VolatilityAnalysis:
    ...
    garch_params: Optional[GARCHParameters]
    regime_params: Optional[RegimeSwitchingParams]
...
class AdvancedVolatilityForecaster:
    ...
    def fetch_historical_data_for_volatility(self, symbol: str, lookback_days: int = 100) -> pd.DataFrame:
        ...
        try:
            ...
            # Generate synthetic realistic data for demonstration
                ...
                ohlc_data = []
                for i, (date, price, vol) in enumerate(zip(dates, prices, volatilities)):
    ...
    def fit_garch_model(self, symbol: str, lookback_days: int = 100) -> Optional[GARCHParameters]:
            ...
            if len(forecasts) >= 2:
                for horizon in self.forecast_horizons:
                    ...
                    if len(relevant_forecasts) >= 2:
                        ...
                        if total_weight > 0:
                            if horizon == 1:
                                ensemble_vol = sum(f.forecast_1d * w for f, w in zip(relevant_forecasts, weights)) / total_weight
                            elif horizon == 5:
                                ensemble_vol = sum(f.forecast_5d * w for f, w in zip(relevant_forecasts, weights)) / total_weight
                            else:
                                ensemble_vol = sum(f.forecast_21d * w for f, w in zip(relevant_forecasts, weights)) / total_weight
