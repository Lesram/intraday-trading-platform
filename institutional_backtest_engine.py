...
try:
    from runfile import ENHANCED_FEATURES, add_technical_indicators, fetch_alpaca_bars
...
class BacktestPerformanceAnalyzer:
    ...
    def analyze_regime_performance(self, returns: pd.Series,
        ...
        try:
            ...
            # Classify regimes
            regimes = []
            for i, (ret, vol) in enumerate(zip(market_returns, rolling_vol, strict=False)):
                if pd.isna(vol):
